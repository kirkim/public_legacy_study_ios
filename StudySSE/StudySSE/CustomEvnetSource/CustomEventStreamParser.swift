...
final class CustomEventStreamParser {
    ...
    func append(data: Data?) -> [CustomEvent] {
        guard let data = data else { return [] }
        dataBuffer.append(data)

        let events = extractEventsFromBuffer().compactMap { [weak self] eventString -> CustomEvent? in
            guard let self = self else { return nil }
            return CustomEvent(eventString: eventString, newLineCharacters: self.validNewlineCharacters)
        }
